...
function ensureHomeBlocks(){
  if (el('homeGamifyReader')) return;

  const card = h(`<section id="homeGamifyReader" class="rounded-xl shadow-card p-4 bg-white mt-4 mb-4">
  ...
  // Try to find the "Top 4" card by its title text
  const titleText = 'รายชั้น: เดือนนี้ออมบ่อยกว่าค่าเฉลี่ย (Top 4)';
  let anchor = null;
  ...
  if (anchor && anchor.parentNode){
    ...
    // Fallback to prepend near the top of home
    const homeLike = document.querySelector('#home, #page-home, main, body');
    homeLike?.insertBefore(card, homeLike.firstChild);
  }

  // Hide legacy localStorage Gamify card if present

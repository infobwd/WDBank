...
function calcFairnessRows(scope, limit=10){
  ...
  const rows = byAcc.map(r=>{
    ...
    return {บัญชี: r.acc, ห้อง: r.cls || '-', คะแนนวินัย: score, ครั้งฝาก: r.depCount, สตรีค: best||0, รวมฝาก: r.depAmt, รวมถอน: r.wdrAmt, สุทธิ: net};
...
function renderLeaderboard(scope, mode){
  ...
  let headers=['ชั้น','ครั้งฝาก','ครั้งถอน','รวมฝาก','รวมถอน','สุทธิ'];
  ...
  if(mode==='fair'){
    ...
    if(!fair.length){ thead.innerHTML = '<th colspan="6">แฟร์ (วินัย)</th>'; body.innerHTML='<tr><td class="sk sk-text" colspan="6">ไม่พบข้อมูล</td></tr>'; return; }
    headers = ['อันดับ','บัญชี','ห้อง','คะแนนวินัย','ครั้งฝาก','สตรีค'];

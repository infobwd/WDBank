...
// Leaderboard + Charts + Academic year + Share
let LB_SELECTED_TERM = "";
...
function getAcademicYears(){ const set=new Set(); (TX||[]).forEach(r=>{ const y=String(r['ปีการศึกษา']||'').trim(); if(y) set.add(y); }); const arr=Array.from(set).map(s=>({s:s,p:parseTermStr(s)})).filter(x=>x.p); arr.sort((a,b)=>(b.p.year-a.p.year)||(b.p.term-a.p.term)); return arr.map(x=>x.s); }
function populateLBYears(){ const sel=document.getElementById('lb-year'); if(!sel) return; const years=getAcademicYears(); years.forEach(s=>{ const o=document.createElement('option'); o.value=s; o.textContent=s; sel.appendChild(o); }); sel.addEventListener('change', ()=>{ LB_SELECTED_TERM = sel.value || ''; const active=document.querySelector('#view-leaderboard .pill.active'); const scope=active?active.dataset.scope:'week'; renderLeaderboard(scope); }); }
function getRangeByScope(scope){ if(scope==='term'){ var t = (LB_SELECTED_TERM? LB_SELECTED_TERM : latestTermFromTX()); return {type:'term',value:t}; } var now=new Date(); if(scope==='week'){ var start=new Date(now); var day=(now.getDay()+6)%7; start.setDate(now.getDate()-day); start.setHours(0,0,0,0); var end=new Date(start); end.setDate(start.getDate()+7); return {type:'range',start:start,end=end}; } var startM=new Date(now.getFullYear(), now.getMonth(), 1, 0,0,0,0); var endM=new Date(now.getFullYear(), now.getMonth()+1, 1, 0,0,0,0); return {type:'range',start:startM,end:endM}; }
...
function buildCharts(scope){ var scopeObj=getRangeByScope(scope);
  // Build GSB account set from AC
  var gsb=new Set(); (Array.isArray(AC)?AC:[]).forEach(a=>{ var isGSB=String(a['ออมสิน']||'').toUpperCase()==='TRUE'; var acc=String(a['รหัสนักเรียน']||a['บัญชี']||'').trim(); if(isGSB && acc) gsb.add(acc); });
  // Aggregate per class
  var byClass=new Map(), byClassG=new Map();
  (TX||[]).forEach(function(r){ if(!inScopeTx(r,scopeObj)) return; var cls=String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ'); var act=String(r['รายการ']||'').trim(); var amt=toNumber(r['จำนวนเงิน']); var acc=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); var o=byClass.get(cls)||{dep:0,wdr:0}; if(act==='ฝาก') o.dep+=isFinite(amt)?amt:0; else if(act==='ถอน') o.wdr+=isFinite(amt)?amt:0; byClass.set(cls,o); if(gsb.has(acc)){ var g=byClassG.get(cls)||{dep:0,wdr:0}; if(act==='ฝาก') g.dep+=isFinite(amt)?amt:0; else if(act==='ถอน') g.wdr+=isFinite(amt)?amt:0; byClassG.set(cls,g); } });
  var labels=Array.from(byClass.keys()); var depArr=labels.map(l=>byClass.get(l).dep); var wdrArr=labels.map(l=>byClass.get(l).wdr); var depG=labels.map(l=> (byClassG.get(l)?byClassG.get(l).dep:0)); var wdrG=labels.map(l=> (byClassG.get(l)?byClassG.get(l).wdr:0));
  var pairs=labels.map((l,i)=>({l:l,sum:depArr[i]+wdrArr[i],dep:depArr[i],wdr:wdrArr[i],depG:depG[i],wdrG:wdrG[i]})); pairs.sort((a,b)=>b.sum-a.sum); pairs=pairs.slice(0,8);
  labels=pairs.map(p=>p.l); depArr=pairs.map(p=>p.dep); wdrArr=pairs.map(p=>p.wdr); depG=pairs.map(p=>p.depG); wdrG=pairs.map(p=>p.wdrG);
  var ctxB=document.getElementById('chartBar').getContext('2d'); if(_barChart){ _barChart.destroy(); } _barChart=new Chart(ctxB,{type:'bar',data:{labels:labels,datasets:[{label:'รวมฝาก',data:depArr},{label:'รวมถอน',data:wdrArr},{label:'ฝาก (ออมสิน)',data:depG},{label:'ถอน (ออมสิน)',data:wdrG}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}},scales:{x:{ticks:{maxRotation:0,autoSkip:true}}}}});
  // Aggregate per day
  var byDay=new Map(), byDayG=new Map(); (TX||[]).forEach(function(r){ if(!inScopeTx(r,scopeObj)) return; var d=parseThaiDate(r['วันที่']); if(!d) return; var k=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'); var act=String(r['รายการ']||'').trim(); var amt=toNumber(r['จำนวนเงิน']); var acc=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); var o=byDay.get(k)||{dep:0,wdr:0}; if(act==='ฝาก') o.dep+=isFinite(amt)?amt:0; else if(act==='ถอน') o.wdr+=isFinite(amt)?amt:0; byDay.set(k,o); if(gsb.has(acc)){ var g=byDayG.get(k)||{dep:0,wdr:0}; if(act==='ฝาก') g.dep+=isFinite(amt)?amt:0; else if(act==='ถอน') g.wdr+=isFinite(amt)?amt:0; byDayG.set(k,g);} });
  var days=Array.from(byDay.keys()).sort(); var depD=days.map(k=>byDay.get(k).dep); var wdrD=days.map(k=>byDay.get(k).wdr); var depDG=days.map(k=> (byDayG.get(k)?byDayG.get(k).dep:0)); var wdrDG=days.map(k=> (byDayG.get(k)?byDayG.get(k).wdr:0));
  var ctxL=document.getElementById('chartLine').getContext('2d'); if(_lineChart){ _lineChart.destroy(); } _lineChart=new Chart(ctxL,{type:'line',data:{labels:days,datasets:[{label:'ฝาก',data:depD},{label:'ถอน',data:wdrD},{label:'ฝาก (ออมสิน)',data:depDG},{label:'ถอน (ออมสิน)',data:wdrDG}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}}}});
}

async function exportLeaderboardPDF(scope){ var titleMap={week:'ลีดเดอร์บอร์ดระดับชั้น • สัปดาห์นี้', month:'ลีดเดอร์บอร์ดระดับชั้น • เดือนนี้', term:'ลีดเดอร์บอร์ดระดับชั้น • เทอม '+(LB_SELECTED_TERM?LB_SELECTED_TERM:'ล่าสุด')}; var rows=aggregateClass(scope); if(!(rows && rows.length)){ alert('ไม่มีข้อมูลสำหรับรายงาน'); return; } var headers=['ชั้น','ครั้งฝาก','ครั้งถอน','รวมฝาก','รวมถอน','สุทธิ']; var dataRows=rows.map(function(r){ return {'ชั้น':r['ชั้น'],'ครั้งฝาก':r['ครั้งฝาก'],'ครั้งถอน':r['ครั้งถอน'],'รวมฝาก':r['รวมฝาก'],'รวมถอน':r['รวมถอน'],'สุทธิ':r.สุทธิ}; }); var wrap=buildPDFShell(titleMap[scope]||'ลีดเดอร์บอร์ดระดับชั้น'); wrap.appendChild(buildPDFTable(headers, dataRows)); addSignatureBlock(wrap); await renderPDF(wrap, 'WDBank-leaderboard-'+scope+'-'+new Date().toISOString().slice(0,10)+'.pdf'); }

async function shareLeaderboard(){ await ensureLogin(); const activeBtn=document.querySelector('#view-leaderboard .pill.active'); const scope=activeBtn?activeBtn.dataset.scope:'week'; const rows=aggregateClass(scope).slice(0,12); if(!(rows && rows.length)){ alert('ไม่มีข้อมูลสำหรับแชร์'); return; } const headers=['ชั้น','ครั้งฝาก','ครั้งถอน','รวมฝาก','รวมถอน','สุทธิ']; const compact=rows.map(r=>({'ชั้น':r['ชั้น'],'ครั้งฝาก':String(r['ครั้งฝาก']),'ครั้งถอน':String(r['ครั้งถอน']),'รวมฝาก':fmtNumber(r['รวมฝาก']),'รวมถอน':fmtNumber(r['รวมถอน']),'สุทธิ':fmtNumber(r.สุทธิ)})); const titleMap={week:'ลีดเดอร์บอร์ด • สัปดาห์นี้', month:'ลีดเดอร์บอร์ด • เดือนนี้', term:'ลีดเดอร์บอร์ด • เทอม '+(LB_SELECTED_TERM?LB_SELECTED_TERM:'ล่าสุด')}; const headerBox={type:'box',layout:'horizontal',contents:headers.map(h=>({type:'text',text:h,size:'xs',weight:'bold',align:'center',flex:1}))}; const dataRows=compact.map((r,i)=>({type:'box',layout:'horizontal',backgroundColor:(i%2?'#FFFFFF':'#F5F6FA'),contents:headers.map(h=>({type:'text',text:r[h],size:'xs',align:'center',flex:1}))})); const bubble={type:'bubble',size:'giga',body:{type:'box',layout:'vertical',contents:[{type:'text',text:titleMap[scope]||'ลีดเดอร์บอร์ด',weight:'bold',size:'lg'},{type:'text',text:'ออมก่อนใช้ • '+thaiDateString(),color:'#7286D3',size:'sm'},{type:'separator',margin:'md'},headerBox,{type:'separator',margin:'sm'},{type:'box',layout:'vertical',margin:'md',spacing:'sm',contents:dataRows}]}}; const flex={type:'flex',altText:titleMap[scope]||'ลีดเดอร์บอร์ด',contents:flexWithOpenButton(bubble)}; await liff.shareTargetPicker([flex]); if(liff.closeWindow) liff.closeWindow(); }
...
function setupLeaderboard(){ populateLBYears(); renderLeaderboard('week'); document.querySelectorAll('#view-leaderboard .pill').forEach(function(btn){ btn.addEventListener('click', function(){ document.querySelectorAll('#view-leaderboard .pill').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); renderLeaderboard(btn.dataset.scope); }); }); var shareBtn=document.getElementById('lb-share'); if(shareBtn){ shareBtn.addEventListener('click', shareLeaderboard); } }

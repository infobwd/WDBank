...
(function(){
  const onReady = ()=>{
    ...
    // Add bottom-nav view switching if present
    document.querySelectorAll('.nav-btn[data-view]').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        const el=$id('view-'+view); if(el) el.classList.add('show');
      });
    });
  };
...
// ==== PATCH v6.6: Leaderboard + Report Hub ====
(function(){
  // Helpers: time ranges
  ...
  function currentTermRange(){
    // Try read from TX['ปีการศึกษา'] like "1/2568" and take the latest term
    const txArr = window.TX||[];
    let latest=null;
    txArr.forEach(r=>{
      const s=String(r['ปีการศึกษา']||'').trim();
      const m=s.match(/^([12])\/(\d{4,4})$/);
      if(m){ const t=+m[1], y=+m[2]; if(!latest || y>latest.y || (y===latest.y && t>latest.t)){ latest={t,y}; } }
    });
    if(latest){
      // Assume term1: May–Oct, term2: Nov–Apr (Thai academic year approx). Adjust if needed.
  ...
  function leaderboardData(mode){
    ...
    const perAcc = new Map(); // acc -> {depCount, depSum, cls}
    sample.forEach(r=>{
      const acc=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); if(!acc) return;
      const cls=String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ');
    ...
    // baseline = avg deposit count per active account in selected range
    ...
    const rows = [...perAcc.entries()].map(([acc,info])=>{
      const score = info.depCount + (info.depCount>=baseline? 0.5:0); // bonus

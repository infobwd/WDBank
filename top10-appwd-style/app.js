...
// ==== Featured Accounts (Top 2) ====
async function renderFeaturedMonthly(){
  await ensureTX(); await ensureAccounts();
  const now=new Date(); const {start,end}=monthRange(now);
  const mapDep=new Map(), mapWdr=new Map();
  (window.TX||[]).forEach(r=>{
    const d=parseThaiDate(r['วันที่']); if(!d) return;
    const acc=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); if(!acc) return;
    if (inRange(d,start,end)){
      if (r['รายการ']==='ฝาก') mapDep.set(acc,(mapDep.get(acc)||0)+1);
      if (r['รายการ']==='ถอน') mapWdr.set(acc,(mapWdr.get(acc)||0)+1);
    }
  });
  // school average (month)
  const depCounts=[...mapDep.values()];
  const schoolAvg = depCounts.length? (depCounts.reduce((a,b)=>a+b,0)/depCounts.length) : 0;

  // candidates: dep>avg and withdraw==0
  const accIndex = new Map();
  (window.ACC||[]).forEach(a=>{
    const key = String(a['รหัสนักเรียน']||a['บัญชี']||'').trim();
    if(key) accIndex.set(key, a);
  });

  const rows=[];
  mapDep.forEach((depCnt, acc)=>{
    if (depCnt > schoolAvg && (mapWdr.get(acc)||0)===0){
      const rec = accIndex.get(acc) || {};
      const bal = toNumber(rec['จำนวนเงินคงเหลือ']||0);
      rows.push({acc, depCnt, bal, cls: rec['ห้อง']||rec['ชั้น']||'-'});
    }
  });

  rows.sort((a,b)=> b.depCnt - a.depCnt || b.bal - a.bal);
  ...
  top2.forEach(r=>{
    ...
    el.innerHTML = `<div class="feat-h">${formatAccountMasked(r.acc)}</div>
      <div class="feat-sub">ชั้น: ${r.cls}</div>
      <div class="feat-sub">ฝากเดือนนี้: <b>${r.depCnt}</b> ครั้ง • ถอน: <b>0</b> ครั้ง</div>
      <div class="feat-sub">ยอดคงเหลือ: <b>${fmtNumber(r.bal)}</b> บาท</div>`;
    grid.appendChild(el);
  });
}

document.addEventListener('DOMContentLoaded', ()=>{
  $id('btnFeaturedInfo')?.addEventListener('click', ()=>{
    const html = [
      '<div style="text-align:left;line-height:1.6">',
      '<b>เกณฑ์คัดเลือก (เดือนนี้)</b><br/>',
      '1) ฝากบ่อยกว่าค่าเฉลี่ยของทั้งโรงเรียน<br/>',
      '2) ยัง <b>ไม่เคยถอน</b> ในเดือนนี้<br/>',
      'เมื่อเข้าเงื่อนไข จะเรียงจาก “จำนวนครั้งฝาก” มาก → น้อย (หากเท่ากันดูยอดคงเหลือ)',
      '</div>'
    ].join('');
    if (typeof Swal!=='undefined' && Swal.fire){
      Swal.fire({title:'2 บัญชีเด่น', html, icon:'info'});
    }else{
      alert('เกณฑ์: ฝากมากกว่าเฉลี่ยของโรงเรียน และไม่มีการถอนในเดือนนี้');
    }
  });
});
...
function computePeriodTop(freq='month'){
  const now=new Date();
  const r = (freq==='week')? weekRange(now) : monthRange(now);
  const {start,end}=r;
  const mapDep=new Map();
  const mapWdr=new Map();
  (window.TX||[]).forEach(rw=>{
    const d=parseThaiDate(rw['วันที่']); if(!d) return;
    const acc=String(rw['บัญชี']||rw['รหัสนักเรียน']||'').trim(); if(!acc) return;
    if (inRange(d,start,end)){
      if (rw['รายการ']==='ฝาก') mapDep.set(acc,(mapDep.get(acc)||0)+1);
      if (rw['รายการ']==='ถอน') mapWdr.set(acc,(mapWdr.get(acc)||0)+1);
    }
  });
  const depCounts=[...mapDep.values()];
  const avgDep = depCounts.length? (depCounts.reduce((a,b)=>a+b,0)/depCounts.length) : 0;
  return {start,end,mapDep,mapWdr,avgDep};
}

async function renderTopBal(freq='month'){
  await ensureTX(); await ensureAccounts();
  const {mapDep, avgDep} = computePeriodTop(freq);
  const idx = new Map();
  (window.ACC||[]).forEach(a=>{
    const key=String(a['รหัสนักเรียน']||a['บัญชี']||'').trim();
  ...
  const rows=[];
  idx.forEach((a,acc)=>{
    const bal = toNumber(a['จำนวนเงินคงเหลือ']||0);
    const dep = mapDep.get(acc)||0;
    if (dep >= avgDep && isFinite(bal)){
      rows.push({acc, bal, dep, cls: a['ห้อง']||a['ชั้น']||'-'});
    }
  });
  ...
  grid.innerHTML = top10.map((r,i)=>`
    <div class="lb-item">
      ...
      <div class="lb-sub">ชั้น: ${r.cls}</div>
      <div class="lb-sub">ยอดคงเหลือ: <b>${fmtNumber(r.bal)}</b> บาท</div>
      <div class="lb-sub">จำนวนครั้งฝาก (${freq==='week'?'สัปดาห์นี้':'เดือนนี้'}): <b>${r.dep}</b> ครั้ง</div>
    </div>
  `).join('');
}

async function renderTopFreq(freq='month'){
  await ensureTX();
  const {mapDep} = computePeriodTop(freq);
  const rows=[...mapDep.entries()].map(([acc,dep])=>({acc,dep}));
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function setFreqPeriod(freq){
    ...
    $id('btnFreqMonth')?.classList.toggle('btn-primary-outline', freq!=='month');
    renderTopFreq(freq);
  }
  $id('btnBalWeek')?.addEventListener('click', ()=> setBalPeriod('week'));
  $id('btnBalMonth')?.addEventListener('click', ()=> setBalPeriod('month'));
  $id('btnFreqWeek')?.addEventListener('click', ()=> setFreqPeriod('week'));
  $id('btnFreqMonth')?.addEventListener('click', ()=> setFreqPeriod('month'));
  // initial
  setBalPeriod('month'); setFreqPeriod('month');
});


document.addEventListener('DOMContentLoaded', ()=>{
  if (typeof ensureTX === 'function') {
    ensureTX().then(()=>{ try{ renderFeaturedMonthly(); }catch(e){} });
  } else {

...
// === Data source (TX) ===
const SHEET_TX_URL = 'https://opensheet.elk.sh/1EZtfvb0h9wYZbRFTGcm0KVPScnyu6B-boFG6aMpWEUo/Sortรายการฝากและถอน';
async function ensureTransactionsLoaded(){
  try{
    if (!Array.isArray(window.TX) || window.TX.length===0){
      const res = await fetch(SHEET_TX_URL + (SHEET_TX_URL.includes('?')?'&':'?') + '_ts=' + Date.now(), {cache:'no-store'});
      const data = await res.json();
      if(Array.isArray(data)) window.TX = data;
    }
  }catch(e){ console.warn('ensureTransactionsLoaded error', e); }
}

// === Monthly Averages (Home card) ===
async function updateMonthlyDeltaFromSheet(){
  try{
    await ensureTransactionsLoaded();
    const txArr = Array.isArray(window.TX)? window.TX : [];
    const now = new Date();
    ...
    txArr.forEach(r=>{
      ...
      if(r['รายการ'] === 'ฝาก'){
        if(d>=start && d<end) mapNow.set(acc, (mapNow.get(acc)||0)+1);
        if(d>=prev.start && d<prev.end) mapPrev.set(acc, (mapPrev.get(acc)||0)+1);
      }
    });
    ...
    ['miniTip','deltaPct','avgThisMonth','avgBaseline'].forEach(id=>{ const el=$id(id); if(el){ el.classList.remove('sk','sk-title'); } });
  }catch(e){
    console.warn('updateMonthlyDeltaFromSheet error', e);
  }
}
...
(function(){
  ...
  // Share leaderboard via LINE (if LIFF available)
  async function shareLeaderboard(){
    ...
    const active = document.querySelector('[data-lb].active') || document.querySelector('[data-lb="week"]');
    const mode = active ? active.getAttribute('data-lb') : 'week';
    ...
    footer:{type:'box',layout:'vertical',spacing:'sm',contents:[{type:'button',style:'primary',action:{type:'uri',label:'เปิด WDBank (LIFF)',uri:'https://liff.line.me/'+(typeof LIFF_ID!=='undefined'?LIFF_ID:'')}}]}};
  ...
  (async ()=>{
    ...
    // Home cards init
    setMiniTip(); updateMonthlyDeltaFromSheet();
  })();
})();

// PATCH v6.6.3: ensure home-only cards are inside #view-home
(function(){
  function relocateHomeCards(){
    const home = document.getElementById('view-home');
    if(!home) return;
    ['card-mini-tips','card-monthly-delta'].forEach(id=>{
      const card = document.getElementById(id);
      if(card && card.parentElement && card.parentElement.id !== 'view-home'){
        home.insertAdjacentElement('afterbegin', card);
      }
    });
  }
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded', relocateHomeCards, {once:true});
  }else{ relocateHomeCards(); }
})();

...
function renderLeaderboard(mode){
  ...
  rows.slice(0,10).forEach((r,i)=>{
    ...
    div.innerHTML = `<div class="lb-rank">${i+1}</div>
      <div class="lb-body"><div class="lb-name">${maskAcc(r.acc)}</div>
      <div class="lb-sub">ชั้น ${r.cls} • ฝาก ${r.depCount} ครั้ง • ${fmtNumber(r.depSum)} บาท</div></div>`;
    grid.appendChild(div);
  });
  const desc=document.getElementById('lb-desc'); if(desc) desc.innerHTML = 'เลือกระยะเวลา: สัปดาห์ / เดือน / เทอม<br><small>แหล่งข้อมูลธุรกรรม: เรียงจากรายการล่าสุดอยู่บนสุด</small><br>เกณฑ์อ้างอิง (ฝากเฉลี่ยต่อบัญชี) = '+(baseline? baseline.toFixed(2):'0');
}
// charts
...
async function shareLeaderboard(){
  try{ if(typeof liff!=='undefined' && typeof LIFF_ID==='string' && LIFF_ID){ try{ await liff.init({liffId:LIFF_ID}); }catch(e){} if(!liff.isLoggedIn()){ await Swal.fire({title:'ต้องเข้าสู่ระบบ LINE',text:'เพื่อแชร์ลีดเดอร์บอร์ด',icon:'info'}); liff.login({redirectUri:location.href}); return; } } }catch(e){}
  ...
  ]}, footer:{type:'box',layout:'vertical',spacing:'sm',contents:[{type:'button',style:'primary',action:{type:'uri',label:'เปิด WDBank (LIFF)',uri:'https://liff.line.me/'+(typeof LIFF_ID==='string'? LIFF_ID:'')}}]}};
  try{ if(typeof liff!=='undefined' && typeof LIFF_ID==='string' && LIFF_ID){ await liff.shareTargetPicker([{type:'flex',altText:'WDBank • ลีดเดอร์บอร์ด',contents:bubble}]); if(liff.closeWindow) liff.closeWindow(); } }catch(e){ console.warn('shareLeaderboard error', e); }
}
function setupNav(){
...
document.addEventListener('DOMContentLoaded', async ()=>{
  try{ setupNav && setupNav(); }catch(e){}
  try{ setMiniTip && setMiniTip(); }catch(e){}
  try{ await loadTX(); }catch(e){}
  try{ computeMonthlyDeltaCard_v67(); }catch(e){}
  try{
    ...
    document.getElementById('share-leaderboard')?.addEventListener('click', shareLeaderboard);
    document.getElementById('dl-reporthub-pdf')?.addEventListener('click', ()=>{
      try{
        ...
        const bar=document.getElementById('barOverall'); if(bar){ const img=bar.toDataURL('image/png',1.0); doc.addImage(img,'PNG',40,80,515,220); }
        const line=document.getElementById('lineTrend'); if(line){ const img2=line.toDataURL('image/png',1.0); doc.addImage(img2,'PNG',40,320,515,220); }
        doc.save('wdbank-report-hub.pdf');
      }catch(e){ console.warn('pdf error', e); }
    });
  }catch(e){ console.warn('bind error', e); }
});

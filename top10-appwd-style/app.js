...
const CLASS_STYLE = { 'ป.1': { bg:'#fef3c7', fg:'#92400e', icon:'fa-child' }, 'ป.2': { bg:'#ecfccb', fg:'#166534', icon:'fa-leaf' }, 'ป.3': { bg:'#dbeafe', fg:'#1e40af', icon:'fa-pencil' }, 'ป.4': { bg:'#fae8ff', fg:'#6b21a8', icon:'fa-puzzle-piece' }, 'ป.5': { bg:'#fde68a', fg:'#92400e', icon:'fa-star' }, 'ป.6': { bg:'#cffafe', fg:'#155e75', icon:'fa-graduation-cap' }, };
...
function renderTable(headEl, bodyEl, rows){
  ...
  rows.forEach(r=>{
    ...
    headers.forEach(h=>{
      ...
      else{ td.textContent=isNumeric(raw)? fmtNumber(raw): String(raw ?? ''); }
      tr.appendChild(td);
    });
...
async function fetchJSON(url, { timeout=12000, retries=2 } = {}){
  const attempt = async () => {
    ...
    try{ const res = await fetch(url + (url.includes('?')?'&':'?') + '_ts=' + Date.now(), { signal: ctrl.signal, cache:'no-store' }); clearTimeout(t);
...
function getClassFromRow(headers, row){ const ch = headers.find(isClassHeader); return ch ? String(row[ch]) : ''; }
function renderAllStars(){
  ...
  intersection.forEach(key=>{
    ...
    card.innerHTML = `
      <div class="class-tag" style="background:${style.bg}; color:${style.fg}">
        <i class="fa-solid ${style.icon}"></i> ${cls}
      </div>
      <div class="star-hdr">
...
function parseThaiDate(s){ try{ const [datePart, timePart='00:00:00'] = String(s).split(',').map(t=>t.trim()); const [d,m,y] = datePart.split('/').map(x=>parseInt(x,10)); const [hh,mm,ss] = timePart.split(':').map(x=>parseInt(x,10)); const gy = (y>2400) ? y-543 : y; return new Date(gy, m-1, d, hh||0, mm||0, ss||0); }catch(e){ return null; } }
...
function renderWeeklyKPIs(){
  ...
  const net = depAmt - wdrAmt; document.getElementById('kpiNet').textContent = fmtNumber(net) + ' บาท';
...
async function loadProfileAvatar(){ try{ await liff.init({ liffId: LIFF_ID }); if (liff.isLoggedIn()){ const p = await liff.getProfile(); if (p?.pictureUrl) document.getElementById('avatar').src = p.pictureUrl; document.getElementById('loginBadge').classList.remove('show'); } else { document.getElementById('loginBadge').classList.add('show'); } }catch(e){ document.getElementById('loginBadge').classList.add('show'); } }
function buildFlexFromRows(title, rows){
  if (!rows?.length) return null; const headers = Object.keys(rows[0]);
  ...
  return { type:'flex', altText:title, contents:{ type:'bubble', size:'giga', body:{ type:'box', layout:'vertical', contents:[
...
async function shareAllStars(){
  ...
  const keysA = new Set(A.map(x=>x.key)); const keysB = new Set(B.map(x=>x.key)); const keysC = new Set(C.map(x=>x.key));
  ...
  const dataRows = inter.map((r,i)=>({
    type:'box', layout:'horizontal', backgroundColor: i%2? '#FFFFFF':'#F5F6FA',
    contents:[ { type:'text', text: cut(formatAccountMasked(r.key),18), size:'xs', align:'center', flex:1 },
...
function buildClassBalanceSummary(){
  ...
  rows.forEach(r=>{
    ...
    obj.ครั้งฝาก += Number.isFinite(depC)? depC:0; obj.ครั้งถอน += Number.isFinite(wdrC)? wdrC:0; obj.บัญชี += 1; result.set(cls, obj);
  });
...
function buildPDFShell(title){
  const wrap = document.createElement('div'); const h1 = document.createElement('h1'); h1.textContent = title; wrap.appendChild(h1);
  const meta = document.createElement('div'); meta.className='meta'; meta.textContent = `WDBank • ออมก่อนใช้ • วันที่ ${thaiDateString()}`; wrap.appendChild(meta); return wrap;
}
async function renderPDF(node, filename){
  ...
  const canvas = await html2canvas(report, { scale: 2, backgroundColor:'#ffffff' }); const imgData = canvas.toDataURL('image/png');
  ...
  if (imgHeight <= pageHeight - margin*2){ pdf.addImage(imgData, 'PNG', margin, margin, imgWidth, imgHeight); }
    ...
    while (sH < canvas.height){
      ...
      ctx.drawImage(canvas, 0, sH, canvas.width, slice, 0, 0, canvas.width, slice); const sliceData = pageCanvas.toDataURL('image/png'); const sliceHpt = slice * ratio;
      pdf.addImage(sliceData, 'PNG', margin, margin, imgWidth, sliceHpt); sH += slice; if (sH < canvas.height) pdf.addPage();
    }
  }
  pdf.save(filename);
}
function buildPDFSection(title, headers, rows){
  const wrap = buildPDFShell(title); const table = document.createElement('table');
  const thead = document.createElement('thead'); const trh = document.createElement('tr'); headers.forEach(h=>{ const th=document.createElement('th'); th.textContent=h; trh.appendChild(th); });
  thead.appendChild(trh); table.appendChild(thead); const tbody = document.createElement('tbody');
  rows.forEach(r=>{ const tr=document.createElement('tr'); headers.forEach(h=>{ const td=document.createElement('td'); const v=r[h]; const text = isAccountHeader(h) ? formatAccountMasked(v) : (isNumeric(v)? fmtNumber(v): String(v ?? '')); td.textContent = text; tr.appendChild(td); }); tbody.appendChild(tr); });
...
async function exportPDF(which){
  ...
  const node = buildPDFSection(title, headers, rows); await renderPDF(node, `WDBank-${which}-${new Date().toISOString().slice(0,10)}.pdf`);
}
async function exportSummaryPDF(kind){
  const {start, end} = parseDateRangeFilter(kind); const wrap = buildPDFShell(kind==='week'?'สรุปรายสัปดาห์':'สรุปรายเดือน'); const insight = buildTxInsightsRange(start,end); if (insight.node) wrap.appendChild(insight.node);
  const classAgg = new Map(); TX.forEach(r=>{
    const d = parseThaiDate(r['วันที่']); if (!inRange(d,start,end)) return; const cls = String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ'); const act = String(r['รายการ']||'').trim(); const amt = toNumber(r['จำนวนเงิน']);
    const obj = classAgg.get(cls) || { ฝาก:0, ถอน:0, ครั้งฝาก:0, ครั้งถอน:0 }; if (act==='ฝาก'){ obj.ฝาก += Number.isFinite(amt)? amt:0; obj.ครั้งฝาก++; } else if (act==='ถอน'){ obj.ถอน += Number.isFinite(amt)? amt:0; obj.ครั้งถอน++; } classAgg.set(cls, obj);
  });
  ...
  for (const [cls,v] of classAgg.entries()){ const tr=document.createElement('tr'); tr.innerHTML = `<td>${cls}</td><td>${fmtNumber(v.ครั้งฝาก)}</td><td>${fmtNumber(v.ครั้งถอน)}</td><td>${fmtNumber(v.ฝาก)}</td><td>${fmtNumber(v.ถอน)}</td><td>${fmtNumber(v.ฝาก - v.ถอน)}</td>`; tbody.appendChild(tr); }
  table.appendChild(thead); table.appendChild(tbody); wrap.appendChild(table); await renderPDF(wrap, `WDBank-summary-${kind}-${new Date().toISOString().slice(0,10)}.pdf`);
}
async function exportClassBalancePDF(){ const wrap = buildPDFShell('ยอดคงเหลือรวมรายชั้น'); const table = buildClassBalanceSummary(); wrap.appendChild(table); await renderPDF(wrap, `WDBank-class-balance-${new Date().toISOString().slice(0,10)}.pdf`); }
function renderLatest10Headers(){ const headers = ['วันที่','บัญชี','รายการ','จำนวนเงิน','ชั้น']; const thead = document.getElementById('th-latest'); thead.innerHTML=''; headers.forEach(h=>{ const th=document.createElement('th'); th.textContent=h; thead.appendChild(th); }); }
document.addEventListener('DOMContentLoaded', ()=>{
  document.getElementById('todayThai').textContent = thaiDateString(); loadAll(); loadProfileAvatar();
  ...
  document.querySelectorAll('.tab').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      btn.classList.add('active'); document.getElementById('panel-'+btn.dataset.tab).classList.add('show');
    });
  });
  renderLatest10Headers();
});

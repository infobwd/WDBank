const LIFF_ID='2005230346-2OVa774O'; // TODO: set your LIFF ID
...
function parseThaiDate(s){ try{ s=String(s); const [dp,tp='00:00:00']=s.split(',').map(x=>x.trim()); const [d,m,y]=dp.split('/').map(n=>parseInt(n,10)); const [hh,mm,ss]=tp.split(':').map(n=>parseInt(n,10)); const gy=(y>2400? y-543:y); return new Date(gy,(m||1)-1,d||1,hh||0,mm||0,ss||0);}catch(e){return null;} }
...
// ---- Mini Tips ----
const MINI_TIPS=['ออมเล็ก ๆ แต่บ่อย ๆ ดีต่อวินัย 💪','ตั้งเวลาออมประจำ จ/พฤ 07:30 ⏰','ตั้งเป้าหมายสั้นรายสัปดาห์ ✨','ออมก่อนใช้ แยกทันทีที่ได้รับเงิน 💼','ชวนเพื่อนทั้งห้องออมพร้อมกัน 🤝'];
...
async function ensureTX(){ if(Array.isArray(TX) && TX.length) return TX; try{ TX = await fetchJSON(SHEET_TX); }catch(e){ console.warn('TX fetch error', e); } return TX||[]; }
...
async function computeMonthlyDeltaCard(){
  ...
  setText('deltaPct',(isFinite(pct)?(pct>=0?'+':'')+pct.toFixed(0)+'%':'-'));
...
// ---- Top frequent month (home) ----
async function renderTopFrequentMonth(){
  await ensureTX();
  const holder=$id('top-frequent-month'); if(!holder) return;
  holder.innerHTML='';
  ...
  (TX||[]).forEach(r=>{
    ...
    const v=map.get(acc)||{cnt:0,sum:0,cls}; v.cnt+=1; v.sum+=isFinite(amt)?amt:0; map.set(acc,v);
  });
  const arr=[...map.entries()].map(([acc,v])=>({acc,cls:v.cls,cnt:v.cnt,sum:v.sum})).sort((a,b)=> b.cnt-a.cnt || b.sum-a.sum).slice(0,5);
  arr.forEach((r,i)=>{
    const div=document.createElement('div'); div.className='lb-card small';
    div.innerHTML='<div class="lb-rank">'+(i+1)+'</div><div class="lb-body"><div class="lb-name">'+formatAccountMasked(r.acc)+'</div><div class="lb-sub">ชั้น '+r.cls+' • ฝาก '+r.cnt+' ครั้ง • '+fmtNumber(r.sum)+' บาท</div></div>';
...
function leaderboardData(mode){ let range; if(mode==='week') range=weekRange(new Date()); else if(mode==='month') range=monthRange(new Date()); else range=currentTermRange(); const sample=filterByRange(TX,'วันที่',range); const perAcc=new Map(); sample.forEach(r=>{ const acc=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); if(!acc) return; const cls=String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ'); const act=String(r['รายการ']||''); const amt=toNumber(r['จำนวนเงิน']); const node=perAcc.get(acc)||{depCount:0,depSum:0,cls}; if(act==='ฝาก'){ node.depCount += 1; node.depSum += isFinite(amt)? amt:0; } perAcc.set(acc,node); }); const counts=[...perAcc.values()].map(v=>v.depCount).filter(v=>v>0); const baseline = counts.length? (counts.reduce((a,b)=>a+b,0)/counts.length) : 0; const rows=[...perAcc.entries()].map(([acc,info])=>({ acc, cls:info.cls, depCount:info.depCount, depSum:info.depSum, score: info.depCount + (info.depCount>=baseline?0.5:0), baseline })).filter(r=>r.depCount>0).sort((a,b)=> b.score - a.score || b.depCount - a.depCount || b.depSum - a.depSum); return {rows, baseline, range}; }

function buildLbClassSelect(){
  const set = new Set(); (TX||[]).forEach(r=>{ const c=String(r['ชั้น']||r['ห้อง']||'').trim(); if(c) set.add(c); });
  const sel = $id('lb-class-select'); if(!sel) return; const cur = sel.value;
  sel.innerHTML = '<option value=\"\">ทุกชั้น</option>' + [...set].sort().map(c=>`<option value="${c}">${c}</option>`).join('');
  if(cur && [...set].includes(cur)) sel.value = cur;
}

function renderLeaderboard(mode){
  const filterClass = $id('lb-class-select')?.value || '';
  const {rows, baseline} = leaderboardData(mode);
  const grid = $id('lb-grid'); if(!grid) return;
  grid.innerHTML = '';
  const list = filterClass? rows.filter(r=>r.cls===filterClass) : rows;
  list.slice(0,10).forEach((r,i)=>{
    const div=document.createElement('div');
    div.className='lb-card' + (filterClass? ' highlight':''); 
    div.innerHTML = '<div class="lb-rank">'+(i+1)+'</div>' +
      '<div class="lb-body"><div class="lb-name">'+ formatAccountMasked(r.acc) +'</div>' +
...
async function shareLeaderboard(){
  ...
  if(!liff.isLoggedIn()){ await Swal.fire({title:'ต้องเข้าสู่ระบบ LINE',text:'เพื่อแชร์ลีดเดอร์บอร์ด',icon:'info',confirmButtonText:'เข้าสู่ระบบ'}); liff.login({redirectUri:location.href}); return; }
...
// --- Extra DOM guards (merged) ---
function safeBind(selector, evt, handler){
  document.querySelectorAll(selector).forEach(el=> el?.addEventListener?.(evt, handler));
}

// ---- Nav & init ----

// Re-bind safely
safeBind('.nav-btn','click', (ev)=>{ const btn=ev.currentTarget; switchView(btn.dataset.view); });
safeBind('[data-lb]','click', (ev)=>{
...
(async function start(){ if(document.readyState==='loading'){ await new Promise(r=>document.addEventListener('DOMContentLoaded', r, {once:true})); }
  try{ await ensureTX(); }catch(e){}
  setMiniTip();
  await computeMonthlyDeltaCard();
  await renderTopFrequentMonth();
  renderLeaderboard('week'); setCharts('week');
  try{ await liff.init({liffId:LIFF_ID}); }catch(e){}
  if(liff.isLoggedIn()){
    try{ const p=await liff.getProfile(); setSrc('avatar', p?.pictureUrl||'./assets/avatar.svg'); $id('loginBadge')?.classList.remove('show'); }catch(e){}
  }else{
    $id('loginBadge')?.classList.add('show');
  }
})();
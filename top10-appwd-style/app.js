...
async function ensureAccounts(){
  try{
    if (Array.isArray(window.ACC) && window.ACC.length) return window.ACC;
    const res = await fetch(window.SHEET_ACCOUNTS, { cache: "no-store" });
    const data = await res.json();
    window.ACC = Array.isArray(data)? data : [];
  }catch(e){
...
async function renderLeaderboardCards(){
  await ensureTX();
  await ensureAccounts();
  const tx = window.TX||[];
  const acc = window.ACC||[];

  // deposit counts per account for week & month
  const now=new Date();
  const wr=weekRange(now), mr=monthRange(now);

  function depCounts(range){
    const map=new Map();
    for (const r of tx){
      if(String(r['รายการ'])!=='ฝาก') continue;
      const d=parseThaiDate(r['วันที่']); if(!inRange(d, range.start, range.end)) continue;
      const a=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); if(!a) continue;
      map.set(a, (map.get(a)||0)+1);
    }
    return map;
  }
  const depW=depCounts(wr), depM=depCounts(mr);

  // average deposit count (per account with any deposit in range)
  function avgFromMap(mp){
    let sum=0, n=0;
    mp.forEach(v=>{ sum+=v; n++; });
    return n? sum/n : 0;
  }
  const avgW = avgFromMap(depW);
  const avgM = avgFromMap(depM);

  // helper: balance by account & class
  function normalizeAccountRow(r){
    const account = String(r['รหัสนักเรียน']||r['บัญชี']||'').trim();
    const bal = toNumber(r['จำนวนเงินคงเหลือ']);
    const cls = String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ');
    return {account, bal, className: cls};
  }
  const accNorm = acc.map(normalizeAccountRow);

  // 1) TOP 10 ยอดเงินสูงสุด (and deposit >= avg)
  function topBalance(mp, avg){
    const ok = [];
    for (const row of accNorm){
      const cnt = mp.get(row.account) || 0;
      if (cnt >= avg && isFinite(row.bal)){
        ok.push({account: row.account, accountMasked: formatAccountMasked(row.account), className: row.className, value: row.bal, valueFmt: fmtNumber(row.bal), count: mp.get(row.account)||0, countFmt: (mp.get(row.account)||0) })
      }
    }
    return ok;
  }

  const topBalW = topBalance(depW, avgW).sort((a,b)=> b.value - a.value).slice(0,10);
  const topBalM = topBalance(depM, avgM).sort((a,b)=> b.value - a.value).slice(0,10);

  // 2) TOP 10 frequent deposit accounts
  function topFreq(mp){
    const rows=[];
    mp.forEach((v,a)=>{
      rows.push({account:a, accountMasked: formatAccountMasked(a), value:v, valueFmt: v, className: (accNorm.find(x=>x.account===a)?.className || '-')});
    });
    rows.sort((a,b)=> b.value - a.value);
    return rows.slice(0,10);
  }
  const topFreqW = topFreq(depW);
  const topFreqM = topFreq(depM);

  // Render
  const hBal=['#','เลขบัญชี','ชั้น','ยอดคงเหลือ','ครั้งฝาก'];
  const hFreq=['#','เลขบัญชี','ชั้น','จำนวนครั้ง','—'];

  const elBW=$id('lbBalW'), elBM=$id('lbBalM'), elFW=$id('lbFreqW'), elFM=$id('lbFreqM');
  if(elBW){ elBW.innerHTML=''; elBW.classList.remove('sk'); elBW.appendChild(renderTable(topBalW, hBal)); }
  if(elBM){ elBM.innerHTML=''; elBM.classList.remove('sk'); elBM.appendChild(renderTable(topBalM, hBal)); }
  if(elFW){ elFW.innerHTML=''; elFW.classList.remove('sk'); elFW.appendChild(renderTable(topFreqW, hFreq)); }
  if(elFM){ elFM.innerHTML=''; elFM.classList.remove('sk'); elFM.appendChild(renderTable(topFreqM, hFreq)); }
}
document.addEventListener('DOMContentLoaded', ()=>{
  if (typeof ensureTX==='function' && typeof ensureAccounts==='function'){
    Promise.all([ensureTX(), ensureAccounts()]).then(()=> renderLeaderboardCards()).catch(()=> renderLeaderboardCards());
  } else {
    try{ renderLeaderboardCards(); }catch(e){}
  }
});



// ==== Featured 3 accounts (all 3 conditions) ====
// Conditions (practical):
//   - ไม่เคยถอน (จำนวนครั้งที่ถอน == 0)
//   - ฝากถี่ ≥ ค่าเฉลี่ยทั้งโรงเรียน (ใช้ค่าใน 'บัญชี' ถ้ามี; ถ้าไม่มี ใช้เดือนนี้)
//   - เลือก Top 3 ตามยอดคงเหลือ
async function renderFeatured3(){
  await ensureAccounts(); await ensureTX();
  const acc = window.ACC||[];
  const tx = window.TX||[];
  const box = $id('featured3'); if(!box) return;

  // baseline deposit count from 'บัญชี' sheet if available, else month
  let depCounts = acc.map(r => Number(String(r['จำนวนครั้งที่ฝาก']||'0').replace(/[, ]/g,''))).filter(x=>isFinite(x));
  let baseline = 0;
  if (depCounts.length){
    const sum = depCounts.reduce((a,b)=>a+b,0);
    baseline = sum / depCounts.length;
  } else {
    const mr=monthRange(new Date());
    const mp=new Map();
    tx.forEach(r=>{
      if(String(r['รายการ'])!=='ฝาก') return;
      const d=parseThaiDate(r['วันที่']); if(!inRange(d,mr.start,mr.end)) return;
      const a=String(r['บัญชี']||r['รหัสนักเรียน']||'').trim(); if(!a) return;
      mp.set(a,(mp.get(a)||0)+1);
    });
    const arr=[...mp.values()]; baseline = arr.length? (arr.reduce((a,b)=>a+b,0)/arr.length):0;
  }

  const rows = acc.map(r=>{
    const account = String(r['รหัสนักเรียน']||r['บัญชี']||'').trim();
    const bal = toNumber(r['จำนวนเงินคงเหลือ']);
    const dep = Number(String(r['จำนวนครั้งที่ฝาก']||'0').replace(/[, ]/g,''));
    const wd  = Number(String(r['จำนวนครั้งที่ถอน']||'0').replace(/[, ]/g,''));
    const cls = String(r['ชั้น']||r['ห้อง']||'ไม่ระบุ');
    return {account, bal, dep, wd, cls};
  });

  const ok = rows.filter(x => isFinite(x.bal) && x.wd===0 && x.dep >= baseline);
  ok.sort((a,b)=> b.bal - a.bal);
  const top3 = ok.slice(0,3);

  box.innerHTML='';
  if (!top3.length){
    box.innerHTML = '<div class="muted small">ยังไม่มีบัญชีที่เข้าเงื่อนไขครบทั้ง 3 ในช่วงนี้ค่ะ</div><div class="muted small">เงื่อนไขโดยสรุป: ไม่เคยถอน + ฝากถี่กว่าค่าเฉลี่ย + ยอดคงเหลือสูง</div>';
    return;
  }
  const grid=document.createElement('div'); grid.className='cd-grid';
  top3.forEach(r=>{
    const el=document.createElement('div'); el.className='cd-item';
    el.innerHTML = `<div class="cd-h">บัญชี ${formatAccountMasked(r.account)}</div>
      <div class="cd-sub">ชั้น ${r.cls}</div>
      <div class="cd-sub">ยอดคงเหลือ: <b>${fmtNumber(r.bal)}</b> บาท</div>
      <div class="cd-sub">จำนวนครั้งฝาก: <b>${r.dep}</b> ครั้ง (≥ ค่าเฉลี่ย ${baseline.toFixed(1)})</div>`;
    grid.appendChild(el);
  });
  box.appendChild(grid);
}
document.addEventListener('DOMContentLoaded', ()=>{
  try{ renderFeatured3(); }catch(e){}
});
